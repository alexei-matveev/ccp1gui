...
#        self.set_parameter('bypass',0)
    ...
    def WriteInput(self,filename=None):
        ...
        self.set_input("input_file",inputf)
        ...
        # Return the filename so that we can let the user know what we've written
        return filename
...
#
    ...
    def find_binary(self):
        """ Return the path to the gamess-uk executable"""
        
        global find_exe

        # Windows
        if sys.platform[:3] == 'win':
            # No path to gamessuk executable found so try to see if the
            ...
            try:
                ...
                if os.access( gamessuk_exe, os.X_OK):
                    print "Using gamess-uk binary: %s" % gamessuk_exe
                    return gamessuk_exe
            except KeyError:
                pass            
            ...
            if gamessuk_exe:
                print "Using gamess-uk binary from users path: %s" % gamessuk_exe
                return gamessuk_exe
            else:
    ...
    def createInput(self):
        ...
        if self.get_parameter("ana_hessian"):
            input_list.append('file ed2 test.ed2 keep\n')

        if (guess_method == 'Dumpfile'):
            input_list.append('restart new\n')

        input_list.append('title\n')
        input_list.append(self.get_title()+'\n')

        if not self.get_parameter('symmetry'):
            input_list.append('nosym\n')
            input_list.append('adapt off\n')

        if self.get_parameter('scf_bypass'):
            if (scftype == "rhf"):
                input_list.append('bypass one\n')
            elif (scftype == "uhf"):
                input_list.append('bypass one two\n')

        input_list.append('mult '+str(self.get_parameter("spin"))+'\n')
        input_list.append('charge '+str(self.get_parameter("charge"))+'\n')
        ...
        self.__write_basis(input_list)

        #self._write_ecp(file)
        self.__write_ecp(input_list)
        ...
        if self.get_parameter('find_ts') and self.get_parameter('pre_ts_hess'):
            ...
            self.__prscf(input_list,scf_method,postscf_method,scftype,dft)
            #self._prguess(file,scf_method,guess_method)
            self.__prguess(input_list,scf_method,guess_method)
            guess_entered=1
            input_list.append('enter\n')
        ...
        if task == MENU_ENER:
            if postscf_method == 'CISD':
                input_list.append('runtype ci\n')
            elif postscf_method == 'CCSD' or postscf_method == 'CCSD(T)':
                input_list.append('runtype ci\n')
            else:
                input_list.append('runtype scf\n')
        elif task == MENU_GRAD:
            input_list.append('runtype gradient\n')
        elif task == 'hessian':
            input_list.append('runtype hessian\n')
        elif task == MENU_OPT:
                ...
                if self.get_parameter('opt_jorgensen'):
                    if self.get_parameter('find_ts'):
                        if self.get_parameter('pre_ts_hess'):
                            input_list.append('runtype saddle jorgensen fcm\n')
                        else:
                            input_list.append('runtype saddle jorgensen\n')
                        powell = self.get_parameter('opt_powell')
                        if powell:
                            input_list.append('powell\n')
                    else:
                        input_list.append('runtype optim jorgensen\n')
                        hessupdate = self.get_parameter('opt_hess_update')
                        if hessupdate != 'default':
                            input_list.append(str(hessupdate)+'\n')

                    #Now write out all other jorgensen options:
                    eigmin = self.get_parameter('opt_min_hess')
                    if eigmin:
                        input_list.append('eigmin '+str(eigmin)+'\n')
                    eigmax = self.get_parameter('opt_max_hess')
                    if eigmax:
                        input_list.append('eigmin '+str(eigmax)+'\n')
                    rfo = self.get_parameter('opt_rfo')
                    if rfo:
                        input_list.append('rfo '+str(self.get_parameter('opt_rfomode'))+'\n')
                    
                else: # we're not using jorgensen
                    if self.get_parameter('find_ts'):
                        if self.get_parameter('pre_ts_hess'):
                            input_list.append('runtype saddle fcm\n')
                        else:
                            input_list.append('runtype saddle\n')
                    else:
                        input_list.append('runtype optim\n')
            else:
                input_list.append('runtype optx\n')

        # Punch settings for this run type
        if task == MENU_OPT:
            input_list.append('punch coor conn title opti mull lowd\n')
        else:
            input_list.append('punch coor conn title mull lowd\n')
        ...
        if task == MENU_OPT:
            ...
            if self.get_parameter('opt_jorgensen'):
                input_list.append('maxjorg '+str(self.get_parameter("max_opt_step"))+'\n')
            else:
                input_list.append('minmax '+str(self.get_parameter("max_opt_step"))+' '
                           +str(self.get_parameter('max_opt_line'))+'\n')
            input_list.append('xtol '+str(self.get_parameter('opt_conv_thsld'))+'\n')
            input_list.append('stepmax '+str(self.get_parameter("max_opt_step_len"))+'\n')
            if self.get_parameter('optimiser') == 'Z-Matrix':
                input_list.append('value '+str(self.get_parameter('opt_value'))+'\n')

        if task == MENU_ENER:
            if postscf_method == 'MCSCF':
                input_list.append('mcscf\n')
                input_list.append('orbital\n')
                input_list.append('<specify your reference configuration here>\n')
                input_list.append('end\n')

        if task == MENU_ENER:
            if postscf_method == 'MRDCI':
                input_list.append('mrdci direct\n')
            elif postscf_method == 'CCSD':
                input_list.append('ccsd\n')
            elif postscf_method == 'CCSD(T)':
                input_list.append('ccsd(t)\n')
        ...
        if self.get_parameter('ed3_keep'):
            if (scf_method == "RHF" or scf_method == "DFT" or 
                scf_method == "Direct RHF" or scf_method == "Direct DFT"):
                input_list.append('enter '+str(self.get_parameter('enter_sect1'))+'\n')
            else:
                input_list.append('enter '+str(self.get_parameter('enter_sect1'))+' '
        ...
        if self.get_parameter("ana_chargeden"):
            ...
            alpha_sections['dens'] = self.__get_next_section()
            if uhf:
                ...
                beta_sections['dens'] = self.__get_next_section()
                beta_titles['totdens'] = "Total Charge Density"
                beta_sections['totdens'] = self.__get_next_section()
        ...
        if self.get_parameter("ana_spinden"):
            if uhf:
                ...
                beta_sections['spindens'] = self.__get_next_section()
        ...
        if self.get_parameter("ana_chargedengrad"):
            ...
            alpha_sections['grad dens'] = self.__get_next_section()
            if uhf:
                ...
                beta_sections['grad dens'] = self.__get_next_section()
                beta_props.append('totgraddens')
                beta_titles['totgraddens'] = "Grad Charge Density (total)"
                beta_sections['totgraddens'] = self.__get_next_section()
        ...
        for n in range(0,6):
            ...
            if self.get_parameter(txt):
                ...
                if uhf:
                    ...
                    alpha_sections[txt1] = self.__get_next_section()

                    txt2 = 'mo '+str(lumoa+n)
                    alpha_props.append(txt2)
                    alpha_sections[txt2] = self.__get_next_section()
                    ...
                    beta_sections[txt1] = self.__get_next_section()

                    txt2 = 'mo '+str(lumob+n)
                    beta_props.append(txt2)
                    beta_titles[txt2] = txt4 +' (beta)' ' [MO '+str(lumob+n)+']'
                    beta_sections[txt2] = self.__get_next_section()
        ...
        for spin in spins:
            ...
            input_list.append('runtype analysis\n')
            ...
            # Punchfile output
            for sect in punch:
                input_list.append('punch eform grid ' + str(sect)+'\n')

            input_list.append('graphics\n')
            input_list.append('gdef\n')
            ...
            self.__write_grid(input_list,grid)

            for prop in props:
                # Load beta component of property for linear combinations (UHF only)
                if prop == 'spindens' or prop == 'totdens':
                    input_list.append('restore data ed3 1 ' + str(beta_sections['dens']) + '\n')
                elif prop == 'totgraddens':
                    input_list.append('restore data ed3 1 ' + str(beta_sections['grad dens']) + '\n')
                elif prop == 'totpote':
                    input_list.append('restore data ed3 1 ' + str(beta_sections['pote']) + '\n')
                elif prop == 'totatom':
                    input_list.append('restore data ed3 1 ' + str(beta_sections['atom']) + '\n')

                input_list.append('calc\n')
                # Build linear combinations (UHF only) or else pass
                # the prop string through to GAMESS-UK
                if prop == 'spindens':
                    input_list.append('type comb ' + str(alpha_sections['dens']) + ' -1.0\n')
                elif prop == 'totdens':
                    input_list.append('type comb ' + str(alpha_sections['dens']) + ' 1.0\n')
                elif prop == 'totgraddens':
                    input_list.append('type comb ' + str(alpha_sections['grad dens']) + ' 1.0\n')
                elif prop == 'totpote':
                    input_list.append('type comb ' + str(alpha_sections['pote']) + ' 1.0\n')
                elif prop == 'totatom':
                    input_list.append('type comb ' + str(alpha_sections['atom']) + ' 1.0\n')
                else:
                    input_list.append('type ' + prop + '\n')
                input_list.append('title\n')
                input_list.append(titles[prop]+'\n')
                if sections.has_key(prop):
                    input_list.append('section '+str(sections[prop])+'\n')

            # Select vectors to analyse
            if spin == 'alpha':
                if uhf:
                    input_list.append('vectors 2\n')
                    input_list.append('enter 2\n')
                elif rohf:
                    # pick canonicalised set
                    input_list.append('vectors 5\n')
                    input_list.append('enter 5\n')
                else:
                    input_list.append('vectors 1\n')
                    input_list.append('enter 1\n')
            else:
                input_list.append('vectors 3\n')
                input_list.append('enter 3\n')
        ...
        if self.get_parameter("ana_pdc"):
            ...
            input_list.append('section %s\n' % (isec,))
            input_list.append('calc\n')
            input_list.append('type dens\n')
            input_list.append('section %s\n' % (isec+1,))
            input_list.append('surface pote %s %f %f\n' % (isec+10,
            ...
            input_list.append('runtype analysis\n')
            input_list.append('punch pdc\n')

            input_list.append('potf %d %d char %f\n' % (isec + 12, isec + 15, self.get_parameter('pdc_charge_constraint')))
        ...
        if self.get_parameter("ana_frequencies"):
            ...
            if not self.get_parameter('symmetry'):
                input_list.append('nosym\n')
                input_list.append('adapt off\n')
            input_list.append('runtype force\n')
            input_list.append('punch normal vibr\n')
            input_list.append('enter 1\n')
...
#        if scf_method == "DFT" or scf_method == "UDFT" or scf_method == "Direct DFT" or scf_method == "Direct UDFT"
        if dft:
            ...
            input_list.append('dft '+str(functional)+'\n')
            input_list.append('dft quadrature '+str(accuracy)+'\n')
            if weights != "default":
                input_list.append('dft weights '+str(weights)+'\n')

            #if radial & angular grids not default specify type and no. points
            if radialgrid != "default":
                input_list.append('dft '+dft_dictionary[radialgrid]+
                           ' '+str(self.get_parameter("dft_radialgridpoints"))+'\n')
            if angulargrid != "default":
                input_list.append('dft '+dft_dictionary[angulargrid]+
                           ' '+str(self.get_parameter("dft_angulargridpoints"))+'\n')

            #if coulomb fitting  add aux basis and scwarz cutoff
            if jfit:
                input_list.append('dft jfit memory \n') #memory is default as GAMESS will
                                                 #recalualte the integrals anyway
                                                 #if they don't fit in memory.
                input_list.append('dft schwartz '+str(self.get_parameter("dft_schwarz"))+'\n')
                input_list.append('dft jbas '+str(self.get_parameter("dft_jbas"))+'\n')
...
#               scf_method == "Direct UHF" or scf_method == "Direct UDFT"):
        ...
        input_list.append('maxcyc '+str(self.get_parameter("scf_maxcyc"))+'\n')
        input_list.append('thresh '+str(self.get_parameter("scf_threshold"))+'\n')

    def __prguess(self,input_list,scf_method,guess_method):

        if guess_method == "Compute":
            input_list.append('vectors '+str(self.get_parameter("guess_comp"))+'\n')
        elif guess_method == "Dumpfile":
            if (scf_method == "RHF" or scf_method == "DFT" or 
                scf_method == "Direct RHF" or scf_method == "Direct DFT"):
                input_list.append('vectors '+str(self.get_parameter("guess_sect1"))+'\n')
            else:
                input_list.append('vectors '+str(self.get_parameter("guess_sect1"))+' '+
            ...
            if (scf_method == "RHF" or scf_method == "DFT" or 
                scf_method == "Direct RHF" or scf_method == "Direct DFT"):
                input_list.append('vectors getq '+str(guess_lfn)+' '+str(guess_blk1)+' '+str(guess_sect1)+'\n')
            else:
                input_list.append('vectors getq '+str(guess_lfn)+' '+str(guess_blk1)+' '+str(guess_sect1)+' '+
                           str(guess_lfn)+' '+str(guess_blk2)+' '+str(guess_sect2)+'\n')


    def __write_grid(self,input_list,grid):
        scale = 1.0 / 0.529177

        if len(grid.dim) == 3:
            input_list.append('type 3d\n')
            input_list.append('points %d %d %d\n' % (grid.dim[0],grid.dim[1],grid.dim[2]))
            input_list.append('size %f %f %f\n' % (scale*grid.axis[0].length(),
                                            scale*grid.axis[1].length(),
                                            scale*grid.axis[2].length()))
            input_list.append('orig %f %f %f\n' % (scale*grid.origin[0],
                                            scale*grid.origin[1],
                                            scale*grid.origin[2]))
            v = grid.axis[0]
            input_list.append('x %f %f %f\n' % (scale*v[0],
                                         scale*v[1],
                                         scale*v[2]))
            v = grid.axis[1]
            input_list.append('y %f %f %f\n'% (scale*v[0],
                                        scale*v[1],
                                        scale*v[2]))
        elif len(grid.dim) == 2:
            input_list.append('type 2d\n')
            input_list.append('points %d %d\n' % (grid.dim[0],grid.dim[1]))
            input_list.append('size %f %f \n' %  (scale*grid.axis[0].length(),
                                           scale*grid.axis[1].length()))
            input_list.append('orig %f %f %f\n' % (scale*grid.origin[0],
                                            scale*grid.origin[1],
                                            scale*grid.origin[2]))
            ...
            input_list.append('type 1d\n')
            input_list.append('points %d\n' % (grid.dim[0],))
            input_list.append('size %f \n' % (scale*grid.axis[0].length()))
            input_list.append('size %f %f %f\n' % (grid.dim[0],grid.dim[1],grid.dim[2]))
            input_list.append('orig %f %f %f\n' % (scale*grid.origin[0],
                                            scale*grid.origin[1],
                                            scale*grid.origin[2]))
    ...
    def __write_basis(self,input_list):
        ...
        if basis:
            ...
            if basis:
                ...
                input_list.append('basis ' + basis+'\n')

    def __write_ecp(self,input_list):
...
##              "Direct RHF", "Direct UHF", "Direct GVB", "Direct DFT", "Direct UDFT"]
 ...
 #       self.optmethod_tool = interfaces.tools.SelectOptionTool(self,'optimiser_method','Method',self.optmethodopts)
        ...
        self.jobname_tool = interfaces.tools.TextFieldTool(self,'name','Job Name')
