...
class DALTONCalc(QMCalc):
    ...
    def makejob(self,writeinput=None,graph=None):
        ...
        exe = self.get_parameter( 'exe' )

        rundaltonscript_cmd = exe + workdirstr + basisstr + keepscratchstr + scratchstr +\
...
#        self.username_tool = TextFieldTool(self,'username','User Name')
        self.daltonexe_tool = FileTool(self,'exe','Dalton script', action='open' )
...
#        self.submission_tool.widget.pack(in_=page.jobgroup.interior())
    ...
    def __initialisetools(self):
        ...
        # See if we can work out where the dalton script lives
        ...
        noscript = None
        if p.error:
            print 'Error trying to locate dalton script '+str(p.error)
            noscript = 1
        else:
            if ( len( p.output ) > 0 ):
                ...
                output = p.output[0]
                exe = output[:-1]
                self.calc.set_parameter( 'exe', exe )
            else:
                noscript = 1
        if noscript:
            self.calc.set_parameter( 'exe','PLEASE SET ME!' )
            self.Info("The location of your dalton script could not be determined\n" +
                      "Please set the location of the script in the job tab.")
    ...
    def __setscratch(self):
        ...
        if ( scratch == 'UNSET' ):
            ...
            from jobmanager import subprocess
            exe = self.calc.get_parameter( 'exe' )
            cmd = exe + " -h"
            p = subprocess.ForegroundPipe(cmd)
            code = p.run()
