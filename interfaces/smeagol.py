...
class SMEAGOLCalc(calc.Calc):
    ...
    def __init__(self, **kw):
        ...
        if rc_vars.has_key('smeagol_input'):
            # User has opened an fdf file so assume this the input
            ifile = rc_vars['smeagol_input']
            self.set_parameter("input_files",[ifile])
    ...
    def makejob(self,writeinput=None,graph=None):
        ...
        if not job:
            ed.Error("SMEAGOL makejob no job returned!")
            return None
        ...
        stdin_file = files[0]
        # hack - nuke the rc_vars
        if rc_vars.has_key('smeagol_input'):
            rc_vars['smeagol_input'] = None
